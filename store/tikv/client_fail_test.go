...
func (s *testClientFailSuite) TestPanicInRecvLoop(c *C) {
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/store/tikv/panicInFailPendingRequests", `panic`), IsNil)
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/store/tikv/gotErrorInRecvLoop", `return("0")`), IsNil)
	...
	c.Assert(failpoint.Disable("github.com/cookieY/tidb/store/tikv/gotErrorInRecvLoop"), IsNil)
	c.Assert(failpoint.Disable("github.com/cookieY/tidb/store/tikv/panicInFailPendingRequests"), IsNil)
	time.Sleep(time.Second)
