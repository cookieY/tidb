...
func (s *testSessionSuite) TestFailStatementCommit(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/session/mockStmtCommitError", `return(true)`), IsNil)
	_, err := tk.Exec("insert into t values (2),(3),(4),(5)")
	c.Assert(err, NotNil)

	c.Assert(failpoint.Disable("github.com/cookieY/tidb/session/mockStmtCommitError"), IsNil)
...
func (s *testSessionSuite) TestFailStatementCommitInRetry(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/session/mockCommitError8942", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/session/mockStmtCommitError", `return(true)`), IsNil)
	_, err := tk.Exec("commit")
	c.Assert(err, NotNil)
	c.Assert(failpoint.Disable("github.com/cookieY/tidb/session/mockCommitError8942"), IsNil)
	c.Assert(failpoint.Disable("github.com/cookieY/tidb/session/mockStmtCommitError"), IsNil)
...
func (s *testSessionSuite) TestGetTSFailDirtyStateInretry(c *C) {
	defer func() {
		c.Assert(failpoint.Disable("github.com/cookieY/tidb/session/mockCommitError"), IsNil)
		c.Assert(failpoint.Disable("github.com/cookieY/tidb/store/tikv/mockGetTSErrorInRetry"), IsNil)
	}()

	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (id int)")

	c.Assert(failpoint.Enable("github.com/cookieY/tidb/session/mockCommitError", `return(true)`), IsNil)
	...
	c.Assert(failpoint.Enable("github.com/cookieY/tidb/store/tikv/mockGetTSErrorInRetry",
